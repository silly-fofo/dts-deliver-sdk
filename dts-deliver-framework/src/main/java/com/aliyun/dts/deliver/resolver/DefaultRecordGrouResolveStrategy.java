...
//        Preconditions.checkState(!uniqueSources.isEmpty());
    ...
    public boolean resolve(DtsMessage dtsMessage) {
        try {
            ...
            switch (record.getOperationType()) {
                ...
                case DELETE:
                case DDL:
                    RecordAppendResult result = handleDMLRecord(dtsMessage);
                ...
                case HEARTBEAT:
                    ...
                    RecordGroup heartbeatRecordGroup = new RecordGroup(uniqueSourceName, RecordBatchType.heartbeat, 0, 1, time.milliseconds());
                    heartbeatRecordGroup.tryAppend(record);
                    checkpointManager.start(heartbeatRecordGroup);
                    break;
                default:
    ...
    protected RecordAppendResult handleDMLRecord(DtsMessage dtsMessage) throws InterruptedException {
        ...
        try {
            ...
            synchronized (dq) {
                ...
                RecordGroup recordGroup = new RecordGroup(uniqueSourceName, RecordBatchType.dml, key, this.groupSizeLimit, time.milliseconds());
